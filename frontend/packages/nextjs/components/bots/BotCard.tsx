...
export const BotCard: React.FC<BotCardProps> = ({ bot, onDelete }) => {
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            ...
            <span className="px-2 py-1 bg-gray-700 text-gray-100 rounded-full text-sm font-medium">{bot.type}</span>
            <span className="px-2 py-1 bg-gray-700 text-gray-100 rounded-full text-sm font-medium">{bot.role}</span>
          </div>
        </div>
        <button
      ...
      {/* Bot Bio */}
      <div className="mb-4">
        <p className="text-sm leading-relaxed text-gray-400">{getMBTIDescription(calculateMBTI(bot))}</p>
      </div>
